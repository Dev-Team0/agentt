...
export default function ChatPage() {
    ...
    const createNewConversation = () => {
        ...
        setMessages([
            {
                role: "assistant",
                content: "Hello! I'm Willow. How can I assist you today?",
    ...
    const updateConversationMessages = (id: string, newMsgs: Message[]) => {
        setConversations((prev) =>
            prev.map((conv) =>
                conv.id === id
                    ? { ...conv, messages: newMsgs, time: "Just now" }
                    : conv
            )
        );
    };
    ...
    const sendMessage = async (messageText?: string): Promise<void> => {
        ...
        if (filesToSend.length > 0) {
            try {
                ...
                for (const file of filesToSend) {
                    ...
                    console.log(
                        `Upload response status: ${res.status} ${res.statusText}`
                    );
                    ...
                    console.log(
                        `Successfully uploaded: ${file.name} -> ${uploadResult.url}`
                    );
                ...
                console.log(
                    `Successfully uploaded ${uploadedFileData.length} files`
                );
                ...
                if (errorMessage.includes("413")) {
                    ...
                    userErrorMessage +=
                        "File type not supported. Please try PDF, Word, or image files.";
        ...
        const title = generateConversationTitle(
            text || uploadedFileData.map((f) => f.name).join(", ")
        );
        let finalMsgs = [...newMsgs];

        try {
            console.log(
                "Sending message to AI with files:",
            ...
            console.log(
                `AI response received. Files processed: ${
    ...
    if (isAuthChecking) {
        return (
            <div
                className={`flex items-center justify-center h-screen ${themeClasses.bg} ${themeClasses.text}`}
            >
    ...
    return (
        <div className={`h-screen flex ${themeClasses.bg} relative`}>
            ...
            >
                ...
                <header
                    className={`border-b px-3 sm:px-5 py-3 flex items-center justify-between shadow-sm ${themeClasses.cardBg} ${themeClasses.border} z-30`}
                >
                    <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                                <h1
                                    className={`font-semibold text-base truncate ${themeClasses.text}`}
                                >
                                    Willow
                                </h1>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className="text-sm px-4 py-2 rounded-lg border border-red-400 hover:border-red-500 text-red-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 flex items-center gap-2 transition-all shadow-sm">
                ...
                {/* Main Chat Content */}
                <div className="flex-1 overflow-y-auto px-3 sm:px-6 py-6 space-y-0">
                ...
                {/* Chat Input Section */}
                <div
                    className={`border-t px-3 sm:px-6 py-4 ${themeClasses.cardBg} ${themeClasses.border}`}
                >
                    <div className="max-w-4xl mx-auto">
                        {messages.length <= 1 && (
                            <div className="mb-4">
                        ...
                        {uploadedFiles.length > 0 && (
                            <div className="mb-3 space-y-2">
                                {uploadedFiles.map((file, index) => (
                                    <div
                                        key={index}
                                        className={`flex items-center justify-between px-4 py-3 rounded-lg ${themeClasses.cardBg} ${themeClasses.border} shadow-sm`}
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${themeClasses.hoverSecondary}`}>
                        ...
                        <div className="flex gap-3 items-end">
                            <div className="flex-1 relative">
                                <textarea
                                    ref={inputRef}
                                    className={`
                        w-full p-4 pr-16 border rounded-2xl resize-none focus:outline-none transition-all text-base
                        min-h-[56px] max-h-[160px] overflow-hidden
                        ${themeClasses.inputBg} ${themeClasses.text} ${themeClasses.border}
                        focus:border-[#1E06BE] focus:ring-2 focus:ring-[#1E06BE]/20
                      ...
                      `}
                                    ...
                                    placeholder={
                                        uploadedFiles.length > 0
                                            ? "Ask about your files or add more context..."
                                            : "Type your message..."
                                    }
                                    disabled={isLoading}
                                />
                            ...
                            <button
                                ...
                                className="bg-gradient-to-r from-[#1E06BE] to-[#2d0bcc] hover:from-[#2d0bcc] hover:to-[#3c0fd9] disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed text-white p-4 rounded-2xl transition-all duration-200 shadow-lg hover:shadow-xl disabled:shadow-none flex items-center justify-center min-w-[56px]"
                                title={
                        ...
                        <p
                            className={`text-xs mt-2 text-center ${themeClasses.textMuted}`}
                        >
